...
private func keyboardBundle() -> Bundle {
    // When running inside a keyboard extension, Bundle.main often points to the extension bundle.
    // If resources are packaged in the extension, this returns them; otherwise, fall back to the app bundle.
    // Prefer the bundle for the current class to be safe.
    return Bundle(for: SpellCheckController.self)
}

struct SpellDataCache {
...
class SpellCheckController{
    ...
    private static var sharedCache: SpellDataCache?
    ...
    init(specialization: Specialization) {
        ...
        // If we have a cached model set, reuse it to avoid re-parsing files on every cold start
        if let cache = SpellCheckController.sharedCache {
            self.autoComplete = cache.autoComplete
            self.correctSpelling = cache.correctSpelling
            self.nextWordPredictor = cache.nextWordPredictor
            return
        }

        // Otherwise load asynchronously and cache when complete
    ...
    func loadData(){
        do {
            guard let path = keyboardBundle().path(forResource: self.filename, ofType: "csv") else {
                print("Warning: Could not find \(self.filename).csv file")
                return
            }
...
//            }
            ...
            data?.components(separatedBy: .newlines).forEach({
                ...
                if(lineItems.count == 2){
                    ...
                    if(frequency > self.autocompleteCutoffFrequency){
                        self.autoComplete = self.autoComplete.inserting(wordObject)
                    }
            ...
            if let bipath = keyboardBundle().path(forResource: "english_bigrams", ofType: "csv") {
            ...
            bigramdata = nil
                ...
                print("Warning: Could not find english_bigrams.csv file in bundle: \(keyboardBundle().bundlePath)")
            }
            
            if let nextwordpath = keyboardBundle().path(forResource: "english_bigram_probabilities", ofType: "csv") {
            ...
            nextwordpathdata = nil
                ...
                print("Warning: Could not find english_bigram_probabilities.csv file in bundle: \(keyboardBundle().bundlePath)")
            }
            
            // Cache the parsed data for subsequent instances within this extension process
            SpellCheckController.sharedCache = SpellDataCache(
                autoComplete: self.autoComplete,
                correctSpelling: self.correctSpelling,
                nextWordPredictor: self.nextWordPredictor
            )
